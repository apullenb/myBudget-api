...
userRouter.post("/register", jsonParser, async (req, res, next) => {
  ...
  const users = await Services.checkForUser(
    req.app.get("db"),
    newUser.user_name
  );
  if (users) {
  ...
  const addUser = await Services.insertUser(
    req.app.get("db"),
    newUser.first_name,
    newUser.user_name,
    bcryptPassword
...
userRouter.post("/login", jsonParser, async (req, res, next) => {
  ...
  const searchUsers = await Services.getUserWithUserName(
    req.app.get("db"),
    loginUser.user_name
  );
...
userRouter.get("/isverified", authorization, async (req, res) => {
  try {
...
userRouter.get("/", authorization, (req, res, next) => {
  const id = req.user;
